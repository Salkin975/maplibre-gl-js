...
export class ColumnarLineBucket implements Bucket {
    ...
    populateLine(featureTable: FeatureTable, options: PopulateParameters, canonical: CanonicalTileID) {
        ...
        if (shouldSort) {
            this.sortSelectionVector(selectionVector, featureTable, lineSortKey);
        }
    ...
    private sortSelectionVector(
        ...
        const sortKeyName: string = sortKey.toString();
        ...
        // Create array of index-value pairs to cache property values
        const pairs: Array<[number, any]> = [];
        let firstValidValue: any = null;

        // Cache all values and find first non-null for type detection
        for (let i = 0; i < selectionVector.limit; i++) {
            const idx = selectionVector.getIndex(i);
            const value = propertyVector.getValue(idx);
            pairs.push([idx, value]);

            if (firstValidValue === null && value != null) {
                firstValidValue = value;
            }
        }

        // Only sort if we have at least one non-null value
        if (firstValidValue === null) {
            return;
        }

        // Choose comparison function based on the type of the first valid value
        let compareFn: (a: [number, any], b: [number, any]) => number;
        ...
        } else if (typeof firstValidValue === 'bigint') {
            compareFn = (a, b) => {
                const aVal = a[1] as bigint | null;
                const bVal = b[1] as bigint | null;
                if (aVal == null && bVal == null) return 0;
                if (aVal == null) return 1;
                if (bVal == null) return -1;
                return aVal < bVal ? -1 : aVal > bVal ? 1 : 0;
            };
        } else if (typeof firstValidValue === 'string') {
            compareFn = (a, b) => {
                const aVal = a[1];
                const bVal = b[1];
                if (aVal == null && bVal == null) return 0;
        ...
        for (let i = 0; i < pairs.length; i++) {
            selectionVector.setIndex(i, pairs[i][0]);
        }
    }
    ...
    addLine(geometryVector: IGeometryVector, startOffset: number, endOffset: number, isPolygon: boolean,
        ...
        // Scale factor to convert MLT tile extent to MapLibre EXTENT (8192)
        const SCALE_FACTOR = EXTENT / tileExtent;
        ...
        const first = startOffset + firstRelative;
        ...
        for (let i = first; i < end; i++) {
            ...
            if (isSharpCorner && i > first) {
                const prevSegmentLength = VectorUtils.dist(currentVertexX, currentVertexY, prevVertexX,
                    prevVertexY);
