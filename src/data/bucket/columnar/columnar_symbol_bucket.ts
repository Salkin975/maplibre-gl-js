...
import {Formatted, type ResolvedImage} from '@maplibre/maplibre-gl-style-spec';
...
import {type FeatureTable, filter, type IGeometryVector, SelectionVector} from '@maplibre/mlt';
 ...
 */
export class ColumnarSymbolBucket implements Bucket {
    ...
    typeMapping: ('Point' | 'LineString' | 'Polygon') [] = ['Point', 'LineString', 'Polygon', 'Point', 'LineString', 'Polygon'];
    ...
    populateColumnar(featureTable: FeatureTable, options: PopulateParameters, canonical: CanonicalTileID) {
        ...
        if(!filterSpecification){
            ...
            console.info('Line layer without filter currently not supported.');
        }
        ...
        for(let i = 0; i < selectionVector.limit; i++){
            ...
            //TODO: also support lines not only points
            const geometryType = geometryVector.geometryType(featureIndex);
            if(geometryType !== 0 && geometryType !== 3){
                // Skip non-point geometries (LineString=1, Polygon=2, MultiLineString=4, MultiPolygon=5)
                continue;
            }

            //TODO: refactor
            const id = idVector.getValue(featureIndex) as number;
            const sourceLayerIndex = this.sourceLayerIndex;

            const evaluationFeature = null;
            let text: Formatted | void;
            if (hasText) {
                // Expression evaluation will automatically coerce to Formatted
                // but plain string token evaluation skips that pathway so do the
                // conversion here.
                const resolvedTokens = layer.getValueAndResolveTokensMlt('text-field', featureTable,
                    featureIndex, canonical, availableImages);
            ...
            if(topologyVector.geometryOffsets){
                const startOffset = topologyVector.geometryOffsets[featureIndex];
                const numPoints = topologyVector.geometryOffsets[featureIndex+1] - startOffset;
                for(let i = 0; i < numPoints; i++){
                    const vertex = geometryVector.getVertex(startOffset + i);
                    points.push(new Point(Math.round(vertex[0] * scaleFactor), Math.round(vertex[1] * scaleFactor)));
                }
            }
            else{
                const vertex = geometryVector.getVertex(featureIndex);
                points.push(new Point(Math.round(vertex[0] * scaleFactor), Math.round(vertex[1] * scaleFactor)));
            }

            const type = this.typeMapping[geometryType];
            const symbolFeature: SymbolFeature = {
                ...
                //TODO: check only used for data driven evaluation?
                properties: {},
                //type: vectorTileFeatureTypes[feature.type],
                //TODO: check if mlt and mvt geometry types match
                type,
                sortKey
            };
    ...
    addSymbols(arrays: SymbolBuffers,
        ...
        for (let i = 0; i < quads.length; i++) {
            ...
            if (i === quads.length - 1 || sectionIndex !== quads[i + 1].sectionIndex) {
                // align with paste-2: no custom globalState object
...
register('ColumnarSymbolBucket', ColumnarSymbolBucket, {omit: ['layers', 'collisionBoxArray', 'features', 'compareText']});
register('ColumnarSymbolBuffers', SymbolBuffers);
register('ColumnarCollisionBuffers', CollisionBuffers);
...
ColumnarSymbolBucket.MAX_GLYPHS = 65535;

ColumnarSymbolBucket.addDynamicAttributes = addDynamicAttributes;
