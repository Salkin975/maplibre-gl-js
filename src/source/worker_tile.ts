...
export class WorkerTile {
    ...
    async parse(data: VectorTileLike, layerIndex: StyleLayerIndex, availableImages: Array<string>, actor: IActor, subdivisionGranularity: SubdivisionGranularitySetting, encoding?: string, featureTables?: FeatureTable[]): Promise<WorkerTileResult> {
        this.status = 'parsing';
        this.data = data;
        ...
        const isMltEncoding = encoding === 'mlt' && featureTables !== undefined;
        ...
        for (const sourceLayerId in layerFamilies) {
            ...
            let rawFeatureTable: FeatureTable | undefined; // Raw columnar data for columnar buckets
            let vectorTileLayer: VectorTileLike['layers'][string]; // Wrapped MVT-like data for traditional buckets
            let sourceLayerIndex: number;

            if (isMltEncoding) {
                // For MLT: keep both raw FeatureTable and wrapped VectorTileLayer
                rawFeatureTable = featureTables.find(f => f.name === sourceLayerId);
                if (!rawFeatureTable) continue;

                // Also get the wrapped layer for traditional buckets
                vectorTileLayer = data.layers[sourceLayerId];
